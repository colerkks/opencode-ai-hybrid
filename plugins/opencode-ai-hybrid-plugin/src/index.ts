...
const HybridArchPlugin: Plugin = async (ctx) => {
  ...
  const arch_status = tool({
    description:
      "Show current Hybrid Architecture status (Global + Skill + Project) with discovered sources and skills.",
    args: {},
    async execute(_args: Record<string, never>, _toolCtx: ToolContext) {
  ...
  const arch_reload = tool({
    description:
      "Reload Hybrid Architecture configuration from disk (Global/Skill/Project) and re-discover skills.",
    args: {},
    async execute(_args: Record<string, never>, _toolCtx: ToolContext) {
  ...
  const arch_init = tool({
    description:
      "Initialize Hybrid Architecture project files (.opencode/hybrid-arch.json, skills.lock.json, commands, and optional built-in skills).",
    args: {
      installBuiltinSkills: tool.schema.boolean().optional(),
      writeCommands: tool.schema.boolean().optional(),
    },
    async execute(
      args: { installBuiltinSkills?: boolean; writeCommands?: boolean },
  ...
  const arch_skill_list = tool({
    description:
      "List discovered skills and which ones are requested by skills.lock.json.",
    args: {},
    async execute(_args: Record<string, never>, _toolCtx: ToolContext) {
      const { state, skills } = cached;
  ...
  const arch_skill_install = tool({
    description:
      "Install a built-in skill into the current project (.opencode/skills/<skill>/SKILL.md) and update skills.lock.json.",
    args: {
      name: tool.schema.string(),
    },
    async execute(args: { name: string }, _toolCtx: ToolContext) {
  ...
  const arch_skill_uninstall = tool({
    description:
      "Uninstall a project skill (.opencode/skills/<skill>) and update skills.lock.json.",
    args: {
      name: tool.schema.string(),
    },
    async execute(args: { name: string }, _toolCtx: ToolContext) {
